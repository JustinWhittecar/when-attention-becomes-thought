...
- Restructured `references.md` by chapter in reading order. Chapter 1 references are now included (previously missing). Chapters 3-9 appear under a clearly-marked "Proposed reading (not yet incorporated)" section; entries graduate above that divider once the relevant chapter is drafted.

### Added
- Repository scaffolding: mdBook structure, chapter files for all 10 chapters, references file, GitHub Actions workflow.
- Chapter 1 and Chapter 2 populated with first-pass content.
- Reading lists and narrative briefs for Chapters 3 through 10.
